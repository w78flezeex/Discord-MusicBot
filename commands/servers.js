...
module.exports = {
    ...
    options: [{
        name: "server",
        description: "Сервер, о котором вы хотите получить информацию.",
        type: 3,
        required: false
    }],
    ...
    run: async (client, interaction) => {
        ...
        if(!client.config.ownerID.includes(interaction?.user?.id)) return interaction.reply({ content: "Команда доступна только владельцу бота.", ephemeral: true }).catch(e => { });
...
if(config.shardManager.shardStatus == true){
            const getServer = async (guildID) => {
             ...
             if (!guild) return interaction.reply({ content: "Я не на том сервере.", ephemeral: true }).catch(e => { });

             const embed = new EmbedBuilder()
                 .setTitle(`Информация и сервере: ${guild.name}`)
                 .setDescription(`> **ID:** \`${guild.id}\`\n> **Участников:** \`${guild.memberCount}\`\n> **Каналов:** \`${guild.channels.length}\`\n> **Ролей:** \`${guild.roles.length}\`\n> **Эмодзи:** \`${guild.emojis.length}\`\n> **Бустов:** \`${guild.premiumSubscriptionCount}\`\n> **Уровень бустов:** \`${guild.premiumTier}\``)
                 .setColor(client.config.embedColor)
                 .setThumbnail(guild.iconURL)
                 //fetch a guild invite url
                 .addFields([
                     { name: "Инвайт", value: guild?.vanityURLCode ? `https://discord.gg/${guild?.vanityURLCode}` : "Not found invite." },
                 ])
                 .setTimestamp()
